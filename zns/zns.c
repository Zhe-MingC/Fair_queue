...
// 这里其实是逻辑plane号， physical plane number
static inline uint64_t zns_get_multichnlway_ppn_idx(NvmeNamespace *ns, uint64_t slba){
    ...
    uint64_t zone_size = NVME_DEFAULT_ZONE_SIZE / ZNS_PAGE_SIZE;        //double check (O) 一个ZONE包含多少个页
    
    uint64_t zidx= zns_zone_idx(ns, slba);                              //double check (O) 
    uint64_t slpa = (slba >> 3) / (ZNS_PAGE_SIZE/MIN_DISCARD_GRANULARITY); // slpa为逻辑页地址 slba为逻辑sector地址，1个sector为512B, slba / page_size_per_lba, lba = 512Byte, slba就是sector相当于除以了
    uint64_t slpa_origin = slpa;     
    slpa = slpa / spp->planes_per_die;  // 这时slpa变为plane地址
    uint64_t num_of_concurrent_zones = (spp->nchnls / spp->chnls_per_zone) * (spp->ways / spp->ways_per_zone); 
    //并行zone的数量, nchnls / chnls_per_zone为当前的channel容纳的zone数量, ways / ways_per_zone为chip容纳的zone数量
    uint64_t BIG_ITER = zidx / num_of_concurrent_zones ; // zone的id / 并行zone数量，这里能算出第几个并行组
    uint64_t BIG_ITER_VAL = zone_size * num_of_concurrent_zones * spp->planes_per_die;  //zone占的实际plane的位置
    uint64_t small_iter = zidx % (spp->nchnls/spp->chnls_per_zone); // 在该并行zone的第几个channel
    uint64_t small_iter_val =((spp->chnls_per_zone) % (spp->nchnls)) * spp->planes_per_die; 
    uint64_t med_iter = (zidx/(spp->nchnls/spp->chnls_per_zone))%((spp->ways/spp->ways_per_zone)); //在该并行zone的第几个chip
    uint64_t med_iter_val = spp->nchnls * spp->ways_per_zone * spp->planes_per_die; // 该ssd中的plane总数 
...
static inline uint64_t zns_advanced_plane_idx(NvmeNamespace *ns, uint64_t slba){
    ...
    struct zns * zns = n->zns;
    struct zns_ssdparams *spp = &zns->sp;
    uint64_t ppn = zns_get_multichnlway_ppn_idx(ns, slba); //获得其plane的号
...
static inline uint64_t zns_get_multiway_chip_idx(NvmeNamespace *ns, uint64_t slba){
    ...
    if (spp->is_another_namespace)
        ...
        return ((ppn/spp->planes_per_die) % (spp->nchnls * spp->ways)); //先算出plane号，再通过相除算出die号，再相余获得第几个die(chip)
    }
}
...
static int zns_init_zone_geometry(NvmeNamespace *ns, Error **errp)
{
    ...
    /* lbasz是什么: 逻辑块是数据的逻辑单元，通常由操作系统和应用程序使用。它是数据的最小可寻址单元，通常以固定大小的字节为单位（例如，512字节）*/
    if (n->zone_size_bs) {
...
static void zns_init_zoned_state(NvmeNamespace *ns)
{
    ...
    for (i = 0; i < n->num_zones; i++, zone++) {
        ...
        zone->d.zslba = start; //zone的开始逻辑地址
        zone->d.wp = start;
        zone->w_ptr = start; // w_ptr和wp有什么区别吗？
        start += zone_size; //这里将每个zone的写指针都设好了
    }
...
static void  zns_init_zone_identify(FemuCtrl *n, NvmeNamespace *ns, int lba_index)
{
    ...
    zns_init_zoned_state(ns); //先初始zone状态
    ...
    id_ns_z->zoc = 0;   // zone open count
    id_ns_z->ozcs = n->cross_zone_read ? 0x01 : 0x00; //Open zone Capability Support
    ...
    ns->id_ns.nsze = cpu_to_le64(n->num_zones * n->zone_size); //nsize
     ...
     * calculated NPDG.
     */
     // npdg：Deallocated Logical Block Entry Group
...
/*
    清除zone的状态，即将zone设置为closed或empty
*/
                                      ...
                                      uint32_t nlb, bool append)
{
    ...
    // 检查写入是否合法，先检查是否越界，再检查如果失败，失败原因是什么
    if (status != NVME_SUCCESS) {
    } else {
        assert(zns_wp_is_valid(zone));
        if (append) { // 如果是append命令
                                     ...
                                     bool failed)
{
    ...
    NvmeZone *zone;
    NvmeZonedResult *res = (NvmeZonedResult *)&req->cqe; // res为返回给CQ的条目
    uint64_t slba;
    uint32_t nlb;
    ...
    zone->d.wp += nlb; //更新写指针，这里nlb中的lb是最小的操作单元

    if (failed) {
        res->slba = 0;
    }

    if (zone->d.wp == zns_zone_wr_boundary(zone)) { //如果是写指针就在边界上，则将其设置为FULL
        switch (zns_get_zone_state(zone)) {
        case NVME_ZONE_STATE_IMPLICITLY_OPEN:
        case NVME_ZONE_STATE_EXPLICITLY_OPEN:
            zns_aor_dec_open(ns);
                                    ...
                                    uint32_t nlb)
{
    ...
    zone->w_ptr += nlb; //这里是将zone的写指针更新位置
...
/* reset对应的zone，先检查是否为空，再通过cb标目进行具体的reset操作 */
...
static uint16_t zns_set_zd_ext(NvmeNamespace *ns, NvmeZone *zone)
{
    ...
    if (state == NVME_ZONE_STATE_EMPTY) {
        ...
        zns_aor_inc_active(ns);
        zone->d.za |= NVME_ZA_ZD_EXT_VALID; //如果一个空zone状态被标记为有某种扩展，则将其设置为关闭
        zns_assign_zone_state(ns, zone, NVME_ZONE_STATE_CLOSED);
        return NVME_SUCCESS;
    }
...
/*
    op_handler_t指向的函数为zns_open_zone, close，offline，reset这种类型的函数，即实际的操作函数，op为operation
*/
                               ...
                               op_handler_t op_hndlr, NvmeRequest *req)
{
    ...
    if (!proc_mask) { //如果是对当前zone进行操作，==0当前zone，其它时候为closed zone，open zone ， read only zone，full zone 
        status = op_hndlr(ns, zone, zns_get_zone_state(zone), req);
    } else {
        if (proc_mask & NVME_PROC_CLOSED_ZONES) { //如果是处理关闭的zone
            QTAILQ_FOREACH_SAFE(zone, &n->closed_zones, entry, next) { //对于每一个关闭的zone做如下操作
        ...
        if (proc_mask & NVME_PROC_READ_ONLY_ZONES) { //这里对read_only_zones的操作就是对所有zone的操作吗
            for (i = 0; i < n->num_zones; i++, zone++) {
                status = zns_bulk_proc_zone(ns, zone, proc_mask, op_hndlr,
                                             req);
                if (status && status != NVME_NO_COMPLETE) {
...
{
    // dw10和dw11其实是slba的大小端
    NvmeNamespace *ns = &n->namespaces[0];
    uint32_t dw10 = le32_to_cpu(c->cdw10);
    uint32_t dw11 = le32_to_cpu(c->cdw11);

    if (!n->zoned) {
        return NVME_INVALID_OPCODE | NVME_DNR; // NVME_DNR是什么意思
    }
    ...
    *slba = ((uint64_t)dw11) << 32 | dw10;
    ...
    // DNR是一种错误标志、表示命令执行失败，主机不应该尝试重新发送
...
static uint16_t zns_zone_mgmt_send(FemuCtrl *n, NvmeRequest *req)
{
    ...
    uintptr_t *resets;
    uint8_t *zd_ext;
    uint32_t dw13 = le32_to_cpu(cmd->cdw13); //cdw13：NVMe命令的第13个Command Dword
    uint64_t slba = 0;
    uint32_t zone_idx = 0;
    uint16_t status;
    uint8_t action;
    ...
    case NVME_ZONE_ACTION_OFFLINE:
        ...
        }//将read_only的zone进行offline操作
        status = zns_do_zone_op(ns, zone, proc_mask, zns_offline_zone, req);
        break;
    case NVME_ZONE_ACTION_SET_ZD_EXT: //设置区域描述符扩展，区域描述符扩展用于指定区域的容量、状态和其它属性
        ...
        status = zns_set_zd_ext(ns, zone); //设置区域的区域描述符扩展
        if (status == NVME_SUCCESS) {
            return status;
        }
        break;
...
static uint16_t zns_zone_mgmt_recv(FemuCtrl *n, NvmeRequest *req)
{
    ...
    req->status = NVME_SUCCESS;
    //传递的是引用，能得到对应zone的slba和zone_idx
    status = zns_get_mgmt_zone_slba_idx(n, cmd, &slba, &zone_idx);
    if (status) {
        return status;
    }
    // zra为NVMe区域报告的类型，zrasf表示过滤条件
    zra = dw13 & 0xff;
    ...
    // zone_entry_sz是单个区域报告的大小包括可选的区域扩展数据
                             ...
                             bool wrz)
{
    ...
    // 根据slba得到zone，再检查该zone是否能够write(slba和nlp是否合法)
    zone = zns_get_zone_by_slba(ns, slba);
...
static int zns_start_ctrl(FemuCtrl *n)
{
    ...
    // zasl_bs：zone append size limit block size：分区追加大小限制块大小
...
static void zns_init(FemuCtrl *n, Error **errp)
{
    ...
    femu_debug("*******************__________DEBUG_________*****************\n\r");
